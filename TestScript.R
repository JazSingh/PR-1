...
svm.linear <- svm(label ~ ., data = digit.trainset.features, kernel = "radial")
...
svm.linear.tune <- tune.svm(digit.trainset.features[, -c(1)], digit.trainset.features[,1],cost=c(2^(-10:5)))
svm.linear.tuned <- svm(label ~ ., data = digit.trainset.features, cost = 0.125, gamma = 0.3333333, kernel = "radial")
...
pixel.svm.linear <- svm(label ~ ., data = digit.trainset, kernel = "radial")
...
pixel.svm.linear.tune <- tune.svm(digit.trainset[, -c(1)],digit.trainset[,1],cost=c(2^(-10:5)))
pixel.svm.linear.tuned <- svm(label ~ ., data = digit.trainset, cost =  0.0009765625, gamma = 0.001412429 , kernel = "radial")
