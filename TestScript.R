...
#svm.linear.tune <- tune.svm(digit.trainset.features[, -c(1)],digit.trainset.features[,1],cost=c(2^(-5:15)))
svm.linear.tuned <- svm(label ~ ., data = digit.trainset.features, cost = 2)
...
pixel.svm.linear.tuned <- svm(label ~ ., data = digit.trainset, cost = 2)
