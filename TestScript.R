...
print("Tune Radial kernel")
pixel.svm.radial.tune <- tune.svm(digit.trainset[, -c(1)],digit.trainset[,1],cost=c(2^(-5:15)), gamma = c(2^(-12:3)))
...
pixel.svm.radial.pred.acc.tune <- sum(diag(table(digit.testset$label,pixel.svm.radial.pred.tuned)))/nrow(digit.testset)
...
print("Tune linear kernel")
pixel.svm.linear.tune <- tune.svm(digit.trainset[, -c(1)],digit.trainset[,1],cost=c(2^(-5:15)))
...
pixel.svm.linear.pred.acc.tune <- sum(diag(table(digit.testset$label,pixel.svm.linear.pred.tuned)))/nrow(digit.testset)
